...
class UNet3D(Base):
	...
	def forward(self, input, coordmap=None):
		...
		# boundary
		boundary = self.upsampling(conv5)
		boundary = torch.cat([boundary, conv4], dim=1)
		conv6 = self.conv6(boundary)
		conv6, mapping6 = self.pe6(conv6)

		boundary = self.upsampling(conv6)
		boundary = torch.cat([boundary, conv3], dim=1)
		conv7 = self.conv7(boundary)
		conv7, mapping7 = self.pe7(conv7)

		boundary = self.upsampling(conv7)
		boundary = torch.cat([boundary, conv2], dim=1)
		conv8 = self.conv8(boundary)
		conv8, mapping8 = self.pe8(conv8)

		boundary = self.upsampling(conv8)

		if (self._coord is True) and (coordmap is not None):
			boundary = torch.cat([boundary, conv1, coordmap], dim=1)
		else:
			boundary = torch.cat([boundary, conv1], dim=1)

		conv9 = self.conv9(boundary)
		conv9, mapping9 = self.pe9(conv9)

		boundary = self.conv10(conv9)
